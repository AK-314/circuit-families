...
def _validate_evaluation_data(
    ...
    if targets.shape[0] != inputs.shape[0]:
        raise ValueError(
            "targets length must equal the number of input examples."
        )
...
def _distribution_divergences(
    ...
    kl_divergence = (
        full_probabilities
        * (full_log_probabilities - masked_log_probabilities)
    ).sum(dim=-1)
    ...
    masked_to_mixture = (
        ...
        * (masked_log_probabilities - log_mixture)
    ).sum(dim=-1)

    jensen_shannon = 0.5 * (
        full_to_mixture + masked_to_mixture
    )
    ...
    *,
    ...
    try:
        for start in range(0, example_count, batch_size):
            ...
            agreement_count += int(
                (full_predictions == masked_predictions).sum().item()
            )
            full_correct_count += int(
                (full_predictions == batch_targets).sum().item()
            )
            masked_correct_count += int(
                (masked_predictions == batch_targets).sum().item()
            )
            ...
            full_cross_entropy_sum += float(
                full_losses.to(torch.float64).sum().item()
            )
            masked_cross_entropy_sum += float(
                masked_losses.to(torch.float64).sum().item()
            )

            kl_values, jensen_shannon_values = (
                _distribution_divergences(
                    full_logits,
                    masked_logits,
                )
            )

            kl_divergence_sum += float(
                kl_values.to(torch.float64).sum().item()
            )
            jensen_shannon_sum += float(
                jensen_shannon_values.to(torch.float64).sum().item()
            )
    ...
    if not all(math.isfinite(value) for value in numeric_values):
        raise FloatingPointError(
            "Mask-evaluation metrics must all be finite."
        )
...
def _phase_checkpoint_records(
    ...
    transitions = checkpoint_manifest.get(
        "formal_transition_checkpoints"
    )
    if not isinstance(transitions, dict):
        raise ValueError(
            "Checkpoint manifest formal_transition_checkpoints "
            "must be a mapping."
        )
    ...
    for record in transitions.values():
        if not isinstance(record, dict):
            raise ValueError(
                "Every formal transition checkpoint must be a mapping."
            )
        records.append(record)

    stable = checkpoint_manifest.get(
...
def load_checkpoint_evaluation_context(
    *,
    ...
    training_manifest_file = (
        repository / "manifests" / f"training_{run_id}.json"
    )
    ...
    recorded_training_manifest = checkpoint_manifest.get(
        "source_training_manifest"
    )
    expected_training_manifest = str(
        training_manifest_file.relative_to(repository)
    )
    _require_equal(
    ...
    if checkpoint_step not in phase_records:
        raise ValueError(
            f"Checkpoint step {checkpoint_step} is not in the frozen "
            "Stage 7 phase grid."
        )
    ...
    if checkpoint_step not in training_records:
        raise ValueError(
            f"Checkpoint step {checkpoint_step} is absent from the "
            "training manifest."
        )
    ...
    task_config = load_config(
        _resolve_path(repository, configs["task"]["path"])
    )
    model_config = load_model_config(
        _resolve_path(repository, configs["model"]["path"])
    )
    training_config = load_training_config(
        _resolve_path(repository, configs["training"]["path"])
    )
    ...
    actual_combined_hash = combined_config_hash(
        {
            "task": task_config,
            "model": model_config,
    ...
    if data.full_inputs is None or data.full_targets is None:
        raise RuntimeError(
            "Validated full lexicographic tensors are unavailable."
        )
    ...
    if not torch.equal(data.full_inputs[:, 0], expected_left):
        raise ValueError(
            "Complete inputs are not in lexicographic left-operand order."
        )

    if not torch.equal(data.full_inputs[:, 1], expected_right):
        raise ValueError(
            "Complete inputs are not in lexicographic right-operand order."
        )

    if not torch.all(data.full_inputs[:, 2] == 113):
        raise ValueError(
            "Complete inputs do not use the frozen equals token."
        )
    ...
    if not torch.equal(data.full_targets, expected_targets):
        raise ValueError(
            "Complete targets do not match modular addition."
        )
